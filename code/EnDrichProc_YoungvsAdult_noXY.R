...
xdat<-read.table("YoungvsAdult_noXY.jn.rnk",header=T,row.names=1)
xdat<-as.matrix(xdat)
zdat<-xdat
...
write.table(reactomeres, "YoungvsAdult_noXY.EnDrichReactomeResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("YoungvsAdult_noXY.EnDrichReactomeResults.pdf") ; plot2DSets(ss,reactome,reactomeres,resrows=1:500) ; dev.off()
...
write.table(msigres, "YoungvsAdult_noXY.EndrichMSigDBResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("YoungvsAdult_noXY.EndrichMSigDBResults.pdf") ; plot2DSets(ss,msigdb,msigres,resrows=1:500) ; dev.off()
...
write.table(encoderes,"YoungvsAdult_noXY.EnDrichEncodeResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("YoungvsAdult_noXY.EnDrichEncodeResults.pdf") ; plot2DSets(ss,encodedb,encoderes,resrows=1:500) ; dev.off()

save.image("YoungvsAdult_noXY.EnDrichAnalysis.Rdata")
