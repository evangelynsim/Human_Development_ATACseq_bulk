...
xdat<-read.table("FetalvsYoung_noXY.jn.rnk",header=T,row.names=1)
xdat<-as.matrix(xdat)
zdat<-xdat
...
write.table(reactomeres, "FetalvsYoung_noXY.EnDrichReactomeResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("FetalvsYoung_noXY.EnDrichReactomeResults.pdf") ; plot2DSets(ss,reactome,reactomeres,resrows=1:500) ; dev.off()
...
write.table(msigres, "FetalvsYoung_noXY.EndrichMSigDBResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("FetalvsYoung_noXY.EndrichMSigDBResults.pdf") ; plot2DSets(ss,msigdb,msigres,resrows=1:500) ; dev.off()
...
write.table(encoderes,"FetalvsYoung_noXY.EnDrichEncodeResult.txt",sep="\t",col.names=T,row.names=F, quote=F)
ss<-apply(zdat,2,rank)
pdf("FetalvsYoung_noXY.EnDrichEncodeResults.pdf") ; plot2DSets(ss,encodedb,encoderes,resrows=1:500) ; dev.off()

save.image("FetalvsYoung_noXY.EnDrichAnalysis.Rdata")
