---
title: "EdgeR, Scatter Plot (1kbp +/- TSS)"
...
In the GEO submission, 2 processed files (1kbp +/- TSS) were uploaded. 
 
1. atac_hum_tss_pe_mapk30_q30.mx.all_unfiltered.csv
2. atac_hum_tss_pe_mapk30_q30.mx.hum.fix_filt.csv
...
rm1 <- read.csv("/group/card2/Evangelyn_Sim/Transcriptome_chromatin_human/Sequencing_ATAC_RNA/GITHUB/Human_Development_ATACseq_bulk/output/atac_hum_tss_pe_mapk30_q30.mx.hum.fix_filt.csv", row.names = 1)
...
for(i in 1:length(contrast.name)){
  ...
  #write.table(res[[i]], file= paste0("edgeR_ATAC_1kbTSS_all_hum_", contrast.name[i] ,".xls"), quote=F, sep="\t", col.names = T, row.names = F)
...
for(i in 1:length(contrast.name_dev)){
  ...
  #write.table(res_dev[[i]], file= paste0("edgeR_ATAC_1kbTSS_all_hum_", contrast.name_dev[i] ,".xls"), quote=F, sep="\t", col.names = T, row.names = F)
...
rm3 = read.delim("/group/card2/Evangelyn_Sim/Transcriptome_chromatin_human/Sequencing_ATAC_RNA/GITHUB/Human_Development_ATACseq_bulk/output/atac_hum_tss_pe_mapk30_q30.mx.chr", header = TRUE, row.names = 1)
...
for(i in 1:length(contrast.name)){
  ...
  res0 = cbind(Name, results, disp, fitted.vals, coefficients)
  res[[i]] = res0[order(res0$adj.p.value),]
  #  write.table(res[[i]], file= paste0("edgeR_ATAC_1kbTSS_all_hum_", contrast.name[i] ,"_noXY.xls"), quote=F, sep="\t", col.names = T, row.names = F)
...
for(i in 1:length(contrast.name_dev)){
  ...
  res0_dev = cbind(Name, results_dev, disp_dev, fitted.vals_dev, coefficients_dev)
  res_dev[[i]] = res0_dev[order(res0_dev$adj.p.value),]
  #write.table(res_dev[[i]], file= paste0("edgeR_ATAC_1kbTSS_all_hum_", contrast.name_dev[i] ,"_noXY.xls"), quote=F, sep="\t", col.names = T, row.names = F)
